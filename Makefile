...
APP_NAME := cross-rs-custom

VCS_URL := https://github.com/cross-rs/cross
VCS_BRANCH ?= main
...
./cross:
	git clone -b "${VCS_BRANCH}" "${VCS_URL}" ./cross

## Source: fetch
src-fetch: ./cross
	git -C ./cross checkout "$(VCS_BRANCH)"
	git -C ./cross pull
	$(eval VCS_HASH := $(shell git -C ./cross rev-parse --verify HEAD))
